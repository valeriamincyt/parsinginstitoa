...
def get_exact_names(name, aliases, acronyms, labels):
    ...
    all_names = [x for x in all_names if not(x.startswith('Department of'))] #cambio operador
...
print("ror (v1.50-2024-07-29-ror-data.json): ")
print(ror.head(1))
...
ror_to_join = ror[['ror_id','name','status','types','aliases','acronyms','labels','city','state','region','country_name']].copy()

ror_to_join.columns = ['ror_id','name','status','types','aliases','acronyms','labels','city','temp_state','temp_region','country']
...
#There are institutions in ROR that are listed as "Withdrawn" or "Inactive". There was some thought to use the old data associated with these 
# ROR IDs and apply them to successors but for this model, we decided to hold off on doing this because we were unsure if there would be a benefit 
# to doing so. Therefore, the code is provided but this data was not used in building the model.
...
print('withdrawn_or_inactive_df.shape')
print(withdrawn_or_inactive_df.shape)

withdrawn_or_inactive_df['successors_parents'] = withdrawn_or_inactive_df['relationships'].apply(get_successors_from_relationships)
...
print('to_add_to_successors.shape: ')
print(to_add_to_successors.shape)

to_add_to_successors['successor_names'] = to_add_to_successors.apply(lambda x: get_exact_names(x['name'],x.aliases,x.acronyms,x.labels),axis=1)

old_name_data = to_add_to_successors.set_index('ror_id')[['successor_names']].to_dict(orient='index')

successor_dict = to_add_to_successors.groupby('successor')['ror_id'].apply(list).reset_index().set_index('successor').to_dict(orient='index')

# Getting ROR String Matching File and Affiliation Dictionary
inst_ror['extra_names'] = inst_ror['ror_id'].apply(get_extra_names)

print('inst_ror.columns : ----------------------------')
print(inst_ror.columns)

#df = inst_ror.apply(lambda x: get_exact_names(x['name'], x.aliases,x.acronyms, x.labels), axis=1)
...
new_affiliation_dict = inst_ror.set_index('affiliation_id')[['display_name','city','state','country','final_names','ror_id','types']].to_dict(orient='index')

with open(f"{curr_model_artifacts_location}full_affiliation_dict.pkl", "wb") as f:
    pickle.dump(new_affiliation_dict, f)

print('Se crea archivo full_affiliation_dict.pkl -------------------')
...
print('city_region_country.shape: ---------------------------------')
print(city_region_country.shape)
...
print('len(list(set(all_countries))) ------------------')
print(len(list(set(all_countries))))
...
print('Se crea/actualiza el archivo affiliation_vocab.pkl')
print('FINALIZADO OK')